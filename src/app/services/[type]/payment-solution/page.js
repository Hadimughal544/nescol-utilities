...
export default function EnergyForm() {
  ...
  const [paymentInputs, setPaymentInputs] = useState({ services: [], turnover: '' });
...
const handleChange = (index, value, field = null) => {
  ...
  const handleSubmit = async () => { 
    ...
    setLoading(true);
        try{
           const res =  await fetch("http://nescolutilities.co.uk/api/paymentsolution", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                            service: type,
                            Payment_service: paymentInputs.services,
                            Annual_turnover: paymentInputs.turnover,
                            Postcode: postcode,
                            Current_Address: selectedAddress,
            ...
            >
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {['POS Machine', 'SoftPos', 'E-Pos', 'Merchant Portal'].map((service) => (
                    <div
                      key={service}
                      onClick={() => toggleService(service)}
                      className={`flex items-center justify-center gap-2 border p-4 rounded-lg text-center cursor-pointer transition hover:bg-pink-500 hover:text-white shadow-xl ${
                        paymentInputs.services.includes(service)
                          ? 'bg-blue-900 text-white'
                          : 'bg-white text-black border-black'
                      }`}
                    >
                      {PaymenticonMap[service]}
                      <span>{service}</span>
                    </div>
                  ))}
                </div>
              ...
              <div>
                ...
                <select
                  value={paymentInputs.turnover}
                  onChange={(e) => handlePaymentChange('turnover', e.target.value)}
                ...
                >
                  ...
                  <option>£200,000 to £350,000</option>
                  <option>£350,000 to £500,000</option>
                  <option>£500,000 to £750,000</option>
                  <option>£750,000 to £1,000,000</option>
